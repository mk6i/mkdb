	...
	for _, row := range rows {
		...
		for _, elem := range selectList {
			...
			case sql.Average:
				...
				newVals = append(newVals, row.Vals[idx].(int))
			case sql.Count:
				// set initial value used for subsequent aggregation step
				count := int(0)
	...
	for _, row := range rows {
		...
		for colIdx, selectCol := range selectList {
			...
			case sql.Count:
				...
				rows[groupKeyIdx].Vals[colIdx] = rows[groupKeyIdx].Vals[colIdx].(int) + row.Vals[colIdx].(int)
			case sql.Average:
				...
				avg := rows[groupKeyIdx].Vals[colIdx].(int) * (counts[countKey] - 1)
				avg += row.Vals[colIdx].(int)
				avg = int(math.Round(float64(avg) / float64(counts[countKey])))
...
func emptyAggregateRow(selectList sql.SelectList, rows []*storage.Row) ([]*storage.Row, error) {
	...
	for _, elem := range selectList {
		...
		case sql.Count, sql.Average:
			row.Vals = append(row.Vals, int(0))
		default:
